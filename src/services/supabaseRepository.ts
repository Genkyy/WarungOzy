...
import { DEFAULT_CATEGORIES } from './indexedDBRepository';
...
export class SupabaseRepository implements DatabaseRepository {
  ...
  async searchMenuItems(query: string): Promise<MenuItem[]> {
    ...
    if (error) {
      console.error('Supabase searchMenuItems error:', error);
      return [];
    }
    return data || [];
  }
  ...
  async findProductByBarcode(barcode: string): Promise<MenuItem | null> {
    ...
    if (error) {
      console.error('Supabase findProductByBarcode error:', error);
      return null;
    }
    return data || null;
  }
  ...
  async createOrder(orderData: CreateOrderDTO): Promise<Order> {
    ...
    if (error || !data) {
      ...
      throw new Error(`Gagal memproses transaksi di database: ${error?.message}`);
    }
  ...
  async voidOrder(id: number | string): Promise<void> {
    ...
    if (error) {
      console.error('Supabase void_order RPC error:', error);
  ...
  async adjustStock(productId: number | string, delta: number, reason: StockMovement['reason']): Promise<void> {
    ...
    if (error) {
      console.error('Supabase adjust_stock RPC error:', error);
